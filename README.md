...
| `NEXT_PUBLIC_CONTACT_FORM_URL` | Googleフォーム等のURL（任意）                                                                    | 法的・ポリシーページに表示するお問い合わせフォームのURL。未設定時はリポジトリの GitHub Issues がフォールバックされます（本番環境では GitHub Actions の Repository Variables に設定することでビルド時に埋め込まれます）。 |
