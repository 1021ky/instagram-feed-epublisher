...
<body>
    <h1>Missing chapter_title placeholder</h1>
        ...
        (book_layout_dir / "layout.html").write_text(invalid_html_content, encoding="utf-8")
        (book_layout_dir / "layout.css").write_text(css_content, encoding="utf-8")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("invalid_layout_fixtures", ValueError, "Invalid layout structure"),
            ("missing_files_fixtures", FileNotFoundError, "Layout HTML file not found"),
        ],
    )
    def test_load_layout_files(self, request, monkeypatch, fixture_name, expected_exception, expected_error_message):
    ...
    def test_load_layout_files_with_missing_css_only(self, tmp_path, monkeypatch):
    ...
    def test_create_epub_with_layout_files(self, tmp_path, monkeypatch, valid_layout_fixtures):
        ...
        # Act - 実行
        with patch("epubkit.builder.epub") as mock_epub, \
            ...
            mock_epub.write_epub.side_effect = lambda name, book, opts: Path(name).write_bytes(b"EPUB")
    ...
    def test_create_epub_with_missing_layout_files(self, tmp_path, monkeypatch):
    ...
    def test_create_epub_with_invalid_layout_template(self, tmp_path, monkeypatch):
...
<body>
    ...
    <p>{css_content}</p>
        ...
        (book_layout_dir / "layout.html").write_text(invalid_html, encoding="utf-8")
        (book_layout_dir / "layout.css").write_text(css_content, encoding="utf-8")
