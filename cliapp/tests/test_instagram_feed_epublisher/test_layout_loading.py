...
from instagram_feed_epublisher.epubkit.builder import _load_layout_files, create_epub
...
def test_create_epub_with_layout_files(
    ...
    mock_epub.write_epub.side_effect = lambda name, book, opts: Path(name).write_bytes(
        b"EPUB"
    )
...
@patch("instagram_feed_epublisher.epubkit.builder._load_layout_files")
...
def test_load_layout_files_with_custom_parameters(tmp_path, monkeypatch):
    ...
    (custom_layout_dir / custom_html_name).write_text(html_content, encoding="utf-8")
    (custom_layout_dir / custom_css_name).write_text(css_content, encoding="utf-8")
